...
rule bcftools_call:
    ...
    threads: 1
    shell:
        "bcftools call --ploidy 1 -vm {input.bcf} -o {output.variants_bcf} && bcftools view -v snps,indels,mnps,ref,bnd {output.variants_bcf} > {output.snp_vcf} && bcftools filter -o {output.filtered_vcf} -i 'QUAL>20 &&DP>5' {output.snp_vcf}"
